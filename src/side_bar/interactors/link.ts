        ...
        if (last_down == DOWN_TYPE.EMPTY) {
            ...
            if( local_board.view.is_link_creating){
                local_board.emit_add_element(new ClientVertexData(server_pos.x, server_pos.y, "", local_board.view), (response) => { 
                    local_board.emit_add_element( new LinkPreData(linkInteractor.indexLastCreatedVertex, response, orientation), () => {} )
                ...
                local_board.view.link_creating_start = pos;
                local_board.view.link_creating_type = orientation;
                local_board.emit_add_element(new ClientVertexData(server_pos.x, server_pos.y, "", local_board.view), (response) => { linkInteractor.indexLastCreatedVertex = response } );
            }
        } 
            ...
            if( local_board.view.is_link_creating){
                local_board.emit_add_element( new LinkPreData(linkInteractor.indexLastCreatedVertex, last_down_index, orientation), () => {} )
                if (key_states.get("Control")){
                    local_board.view.is_link_creating = true;
    ...
    linkInteractor.mouseup = ((canvas, ctx, g: ClientGraph, e) => {
        ...
        if (vertexIndex != null){
            if ( firstVertexIndex != vertexIndex) { // there is a vertex nearby and it is not the previous one
                local_board.emit_add_element(new LinkPreData(firstVertexIndex, vertexIndex,  orientation), (response: number) => {});
            } 
        } else {
            ...
            if (typeof link == "undefined"){
                ...
                local_board.emit_add_element(new ClientVertexData(server_pos.x, server_pos.y, "", local_board.view), (response) => { 
                    ...
                    local_board.emit_add_element( new LinkPreData(firstVertexIndex, response, orientation), () => {} )
                });
            }
            else {
